...
export default function CartPage() {
    ...
    const runValidation = useCallback(async () => {
        ...
        try {
            ...
            const ids = new Set<string>(
                (data.invalid_items ?? []).map((i: InvalidItem) => i.id),
            );
            setInvalidIds(ids);
    ...
    const handleRemoveAllInvalid = async () => {
        ...
        setInvalidItems([]);
        toast.success(
            `${ids.length} unavailable item${ids.length > 1 ? "s" : ""} removed`,
        );
    };
                    ...
                    <div className="flex-1 min-w-0">
                        ...
                        {!allInvalid && (
                            <p className="text-xs text-amber-700 mt-0.5">
                                You can still checkout with the remaining
                                available items.
                            </p>
                        )}
                    </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-8">
                ...
                <div className="lg:col-span-2 space-y-4">
                    {items.map((item) => {
                        ...
                        const reason = invalidItems.find(
                            (i) => i.id === item.id,
                        )?.reason;
                        return (
                            <div
                                key={item.id}
                                className={`card p-3 sm:p-4 flex gap-3 sm:gap-4 transition-all duration-300 ${
                            ...
                            >
                                ...
                                <div className="flex-1 min-w-0">
                                    ...
                                    {/* Controls row — on mobile shown below name */}
                                    <div className="flex items-center justify-between mt-2">
                                        <div
                                            className={`flex items-center gap-1 border border-gray-200 rounded-lg ${unavailable ? "opacity-40 pointer-events-none" : ""}`}
                                        >
                                            <button
                                                onClick={() =>
                                                    item.quantity > 1 &&
                        ...
                        <div className="space-y-2 text-sm mb-4">
                            ...
                            {hasInvalid && (
                                <div className="flex justify-between text-xs text-red-500">
                                    <span>
                                        Unavailable ({invalidIds.size} item
                                        {invalidIds.size > 1 ? "s" : ""}{" "}
                                        excluded)
                                    </span>
                                    <span>—</span>
