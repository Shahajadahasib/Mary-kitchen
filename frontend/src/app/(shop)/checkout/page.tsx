...
function AddressCard({
    ...
    return (
        ...
        >
            ...
            <div
                className="flex flex-col gap-1 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity"
...
export default function CheckoutPage() {
    ...
    const [deliveryFeeState, setDeliveryFeeState] = useState<DeliveryFeeState>({
        status: "idle",
    });
            ...
            api.get("/cart/validate/")
                .then(({ data }) => {
                    const ids = new Set<string>(
                        (data.invalid_items ?? []).map(
                            (i: any) => i.id as string,
                        ),
                    );
                    setInvalidIds(ids);
    ...
    const { data: addresses } = useQuery<any>({
        ...
        onSuccess: (data: any) => {
            if (!selectedAddress) {
                const def = (data?.results ?? data)?.find(
                    (a: any) => a.is_default,
                );
    ...
    const selectedAddr = useMemo(
        () =>
            selectedAddress
                ? (addrList.find((a: any) => a.id === selectedAddress) ?? null)
                : null,
    ...
    const validCartItems = (cart?.items ?? []).filter(
        (item: any) => !invalidIds.has(item.id),
    );
    ...
    const calculateFee = useCallback(
        async (addressId: string, orderTotal: number) => {
        ...
        [addrList],
    ); // addrList reference updates when addresses load
    ...
    const deliveryFeeReady =
        orderType === "pickup" || deliveryFeeState.status === "ok";
    ...
    const handlePlaceOrder = async () => {
        ...
        if (!canCheckout) {
            toast.error(
                "All items in your cart are currently unavailable. Please remove them first.",
            );
            return;
        }
        setLoading(true);
        try {
            ...
            if (!checkoutUrl) {
                toast.error(
                    "Checkout succeeded but Stripe payment link is missing. Please try again.",
                );
                await fetchCart();
                return;
            }
                            ...
                            {addrList.length === 0 ? (
                                <div className="text-center py-8 border-2 border-dashed border-gray-200 rounded-xl">
                                    ...
                                    <button
                                        ...
                                        className="btn-primary text-sm py-2 px-4 flex items-center gap-1.5 mx-auto"
                                    >
                                    ...
                                    {deliveryFeeState.status === "ok" && (
                                        <div className="flex items-center gap-1.5 text-xs text-green-700 bg-green-50 border border-green-200 rounded-lg px-3 py-2">
                                            ...
                                            <span>
                                                ...
                                                {deliveryFeeState.distanceKm >
                                                    0 && (
                                                    <>
                                                        {" "}
                                                        ·{" "}
                                                        {deliveryFeeState.distanceKm.toFixed(
                                                            1,
                                                        )}{" "}
                                                        km
                                                    </>
                                                ...
                                                · Est.{" "}
                                                {deliveryFeeState.estimatedDays}{" "}
                                                day
                                                {deliveryFeeState.estimatedDays !==
                                                1
                                                    ? "s"
                        ...
                        <div className="space-y-2 text-sm mb-4 max-h-48 overflow-y-auto pr-1">
                            {(cart?.items ?? []).map((item: any) => {
                                ...
                                return (
                                    <div
                                        key={item.id}
                                        className={`flex justify-between ${isInvalid ? "text-gray-300 line-through" : "text-gray-600"}`}
                                    >
                                ...
                                >
                                    ...
                                    {loading
                                        ...
                                        : deliveryFeeState.status === "loading"
                                          ? "Calculating fee…"
                                          : invalidCount > 0
                                            ? "Place Order (available items)"
                                            : "Place Order"}
                                </button>
                                <p className="text-xs text-gray-400 text-center mt-2">
                                    You will leave this site to pay on Stripe’s
                                    secure checkout page (Powered by Stripe).
                                </p>
                            </>
                        )}
                    </div>
