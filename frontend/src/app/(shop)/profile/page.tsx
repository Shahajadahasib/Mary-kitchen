...
import { User, MapPin, Heart, Package, Plus, Trash2, Edit, Check, Star } from "lucide-react";
...
export default function ProfilePage() {
  ...
  useEffect(() => {
    if (user) setProfileForm({ first_name: user.first_name, last_name: user.last_name, phone_number: user.phone_number || "" });
  }, [user]);
  ...
  if (!user) return (
    <div className="container-xl py-20 text-center">
      <Link href="/login" className="btn-primary">Please Login</Link>
    </div>
  );
  ...
  return (
    <div className="container-xl py-8">
      ...
      <div className="flex gap-1 bg-gray-100 rounded-xl p-1 w-fit mb-6">
        {TABS.map((tab) => (
          <button key={tab} onClick={() => setActiveTab(tab)}
            className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${activeTab === tab ? "bg-white text-primary-700 shadow-sm" : "text-gray-600 hover:text-gray-900"}`}>
            {tab}
          </button>
        ))}
      </div>
      ...
      {activeTab === "Profile" && (
        <div className="max-w-lg card p-6">
          <div className="flex items-center justify-between mb-5">
            <h2 className="font-semibold text-gray-900 flex items-center gap-2"><User className="w-5 h-5 text-primary-600" /> Profile Info</h2>
            <button onClick={() => setEditingProfile(!editingProfile)} className="btn-secondary text-sm py-1.5 px-4 flex items-center gap-1">
              <Edit className="w-3.5 h-3.5" /> {editingProfile ? "Cancel" : "Edit"}
            </button>
          </div>
          {editingProfile ? (
            <div className="space-y-4">
              ...
              <div>
                ...
                <input value={profileForm.phone_number} onChange={(e) => setProfileForm({ ...profileForm, phone_number: e.target.value })} className="input-field" />
              </div>
              <button onClick={handleSaveProfile} className="btn-primary flex items-center gap-2"><Check className="w-4 h-4" /> Save Changes</button>
            </div>
          ) : (
      ...
      {activeTab === "Addresses" && (
        <div className="max-w-2xl">
          <div className="grid gap-4">
            {(addresses?.results ?? addresses ?? []).map((addr: any) => (
                ...
                <div className="flex items-start justify-between">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-700">{addr.full_name}{addr.phone ? ` · ${addr.phone}` : ""}</p>
      ...
      <AddressFormModal
        ...
        onSaved={() => qc.invalidateQueries({ queryKey: ["addresses"] })}
        ...
        <div>
          {wishlist?.items?.length === 0 ? (
            <div className="text-center py-12 text-gray-400">
              ...
              <Link href="/products" className="btn-primary inline-flex mt-4 text-sm">Browse Products</Link>
            </div>
          ) : (
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
              {wishlist?.items?.map((item: any) => (
                <Link key={item.id} href={`/products/${item.product_slug}`} className="card p-4 hover:shadow-md">
                  <p className="font-medium text-sm text-gray-900 line-clamp-2 mb-1">{item.product_name}</p>
                  <p className="text-primary-700 font-bold text-sm">${item.product_price}</p>
                </Link>
              ))}
            </div>
          )}
      ...
      {activeTab === "Orders" && (
        <div className="text-center py-6">
          ...
          <Link href="/orders" className="btn-primary inline-flex">Go to Orders</Link>
        </div>
      )}
    </div>
  );
