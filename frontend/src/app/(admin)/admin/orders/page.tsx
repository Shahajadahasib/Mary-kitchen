...
function AdminOrdersPageInner() {
    ...
    const [refundMode, setRefundMode] = useState<"items" | "amount" | "full">(
        "items",
    );
    ...
    useEffect(() => {
        ...
        let cancelled = false;
        api.get(`/orders/admin/orders/${encodeURIComponent(orderNumber)}/`)
            .then((res) => {
                if (cancelled) return;
    ...
    const handleStatusUpdate = async () => {
        ...
        try {
            await api.post(
                `/orders/admin/orders/${selectedOrder.order_number}/status/`,
                {
                    status: newStatus,
                    note,
                    force: forceUpdate,
    ...
    const selectedRefundTotal = (): number => {
        ...
        return refundOrder.items
            ...
            .reduce((sum: number, it: any) => {
                const q = Math.max(
                    0,
                    it.quantity - (it.refunded_quantity ?? 0),
                );
    ...
    const handleRefund = async () => {
        ...
        if (refundMode === "items") {
            ...
            if (!refundAmountAud || isNaN(aud) || aud <= 0) {
                toast.error("Enter a valid refund amount in AUD");
                return;
            }
        ...
        try {
            ...
            toast.error(
                e?.response?.data?.message ||
                    "Refund failed. Check Stripe or order status.",
            );
        } finally {
            setRefunding(false);
        }
                        ...
                        <thead>
                            <tr className="bg-gray-50 border-b border-gray-100">
                                {[
                                    ...
                                    <th
                                        key={h}
                                        className={`px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wide whitespace-nowrap ${h === "Date" ? "hidden md:table-cell" : ""}`}
                                    >
                                        {h}
                                    </th>
                                ))}
                            ...
                            {isLoading ? (
                                ...
                                orders.map((order: any) => (
                                    ...
                                    >
                                        ...
                                        <td className="px-4 py-3.5">
                                            <div className="flex items-center gap-2.5">
                                                ...
                                                <div className="min-w-0">
                                                    ...
                                                    <p className="text-xs text-gray-400 truncate max-w-[140px] hidden sm:block">
                                                        {order.user_email}
                                                    </p>
                                                </div>
                                            </div>
                                        ...
                                        {/* Date */}
                                        <td className="px-4 py-3.5 text-gray-500 whitespace-nowrap text-xs hidden md:table-cell">
                                            ...
                                            <div className="flex items-center gap-1.5">
                                                {![
                                                    ...
                                                    <button
                                                        ...
                                                        className="inline-flex items-center gap-1 text-xs bg-primary-50 text-primary-700 hover:bg-primary-100 px-2 sm:px-3 py-1.5 rounded-lg font-medium transition-colors whitespace-nowrap"
                                                    >
                                                        Update
                                                    </button>
                                                )}
                            ...
                            {getAllowedNextStatuses(
                                ...
                                <div className="rounded-xl border-2 border-dashed border-orange-200 bg-orange-50 p-5 text-center">
                                    ...
                                    <p className="text-sm font-semibold text-orange-900 mb-1">
                                        Order reached final status
                                    </p>
                                    <p className="text-xs text-orange-600 mb-4 leading-relaxed">
                                        No further transitions are allowed.
                                        Enable force jump to manually override.
                                    </p>
                                    <button
                                        onClick={() => {
                                            setForceUpdate(true);
                                ...
                                <>
                                    ...
                                    >
                                        ...
                                        <div>
                                            ...
                                            <p className="text-xs text-orange-500">
                                                Bypass normal transition rules
                                            </p>
                                        </div>
                                    </label>
                                    ...
                                    <div>
                                        ...
                                        <div className="flex items-center gap-2 mb-2">
                                            ...
                                            {newStatus && (
                                                <span
                                                    className={`badge ${getStatusColor(newStatus)}`}
                                                >
                                                    {orderStatusLabel(
                                                        newStatus,
                                                        selectedOrder.order_type,
                        ...
                        <div className="overflow-y-auto flex-1 px-6 py-5 space-y-5">
                            ...
                            <div>
                                ...
                                <div className="flex gap-2">
                                    {(["items", "amount", "full"] as const).map(
                                        (mode) => (
                                            <button
                                                key={mode}
                                                type="button"
                                ...
                                <div>
                                    ...
                                    <div className="rounded-xl border border-gray-200 overflow-hidden divide-y divide-gray-100">
                                        {refundOrder.items.map((item: any) => {
                                            ...
                                            const fullyRefunded =
                                                refundableQty === 0;
                                            const checked = selectedItemIds.has(
                                                String(item.id),
                                            );
                                            return (
                                                <label
                            ...
                            <div className="flex items-start gap-2.5 bg-amber-50 border border-amber-200 rounded-xl px-4 py-3">
                                ...
                                <p className="text-xs text-amber-800 leading-relaxed">
                                    This triggers a real Stripe refund and{" "}
