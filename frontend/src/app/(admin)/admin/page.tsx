...
export default function AdminDashboardPage() {
    ...
    const { data: orders } = useQuery({
        ...
        queryFn: () =>
            api.get("/orders/admin/orders/?page_size=5").then((r) => r.data),
    ...
    const { data: conversion } = useQuery<ConversionStats>({
        ...
        queryFn: () =>
            api.get(`/analytics/conversion/?days=${days}`).then((r) => r.data),
        refetchInterval: 60_000,
    });
    ...
    const { data: lowStockProducts } = useQuery<LowStockProduct[]>({
        ...
        queryFn: () =>
            api
                .get("/products/admin/low-stock/?threshold=0&limit=8")
                .then((r) => r.data),
        refetchInterval: 30_000,
                ...
                {STATS.map(({ label, value, sub, icon: Icon, color }) => (
                    ...
                    >
                        <div className="flex items-center justify-between mb-2 sm:mb-3">
                            <div
                                className={`w-8 h-8 sm:w-10 sm:h-10 ${color} rounded-lg flex items-center justify-center text-white`}
                            >
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6 mb-4 sm:mb-6">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        ...
                        <div className="bg-orange-50 rounded-xl p-4">
                            ...
                            <p className="text-3xl font-bold text-orange-700">
                                {refundStats
                                    ? formatCurrency(
                                          refundStats.total_refunded_amount,
                                      )
                                    : "—"}
                            </p>
                ...
                <div className="bg-white rounded-xl border border-gray-100 shadow-sm">
                    ...
                    {!refundStats ? (
                        ...
                        <p className="text-sm text-gray-400 text-center py-8">
                            No refunded products in this period
                        </p>
                    ) : (
                        <div className="overflow-x-auto">
                            <table className="w-full text-sm">
                                ...
                                <tbody className="divide-y divide-gray-50">
                                    {refundStats.top_refunded_products.map(
                                        (p) => (
                                            <tr
                                                key={p.product_id}
                                                className="hover:bg-gray-50 transition-colors"
                        ...
                        <tbody className="divide-y divide-gray-50">
                            {recentOrders.map((order: any) => (
                                <tr
                                    ...
                                    className="hover:bg-gray-50 transition-colors"
                                >
                                    <td className="px-5 py-3 font-mono font-medium text-primary-700">
                                        #{order.order_number}
                                    </td>
                                    <td className="px-5 py-3 text-gray-700">
                                        {order.user_email}
