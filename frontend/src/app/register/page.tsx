...
export default function RegisterPage() {
    ...
    const [fieldErrors, setFieldErrors] = useState<
        Partial<Record<(typeof FIELD_KEYS)[number] | "form", string>>
    >({});
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const res = (
                e as { response?: { data?: unknown; status?: number } }
            )?.response;
            const body = res?.data;
            const parsed = parseApiFieldErrors(body);
            const next: Partial<
                Record<(typeof FIELD_KEYS)[number] | "form", string>
            > = {};
            ...
            const legacy = (body as { errors?: { message?: string }[] })
                ?.errors;
            if (
                legacy?.length &&
                legacy[0]?.message &&
        ...
        <div className="min-h-screen flex items-center justify-center bg-gray-50 px-4 py-8">
            <div className="w-full max-w-md">
                <div className="text-center mb-8">
                    <Link
                        href="/"
                        className="inline-flex items-center gap-2 text-2xl font-bold text-primary-700"
                    >
