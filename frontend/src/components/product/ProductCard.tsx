...
export default function ProductCard({ product }: ProductCardProps) {
    ...
    const cartItem = cart?.items?.find(
        (i) => i.product === product.id && !i.variant,
    );
        ...
        >
            ...
            <div className="relative overflow-hidden rounded-t-xl bg-gray-100 h-36 sm:h-44 md:h-48">
                ...
                {!product.is_in_stock && (
                    <div className="absolute inset-0 bg-black/30 flex items-end">
                        <span className="w-full bg-orange-500/90 text-white text-xs font-medium text-center py-1.5">
                            Out of Stock
                        </span>
                    </div>
                )}
                <div className="absolute top-2 right-2 flex flex-col gap-1 opacity-0 [@media(hover:hover)]:group-hover:opacity-100 transition-opacity">
                    ...
                    >
                        <Heart className="w-3 h-3 sm:w-4 sm:h-4 text-gray-500 hover:text-red-500" />
                    </button>
                </div>
            </div>
            ...
            <div className="p-2 sm:p-3 flex flex-col flex-1">
                ...
                <div className="flex items-center gap-1 mt-1 mb-1.5">
                    <div className="flex">
                        {[1, 2, 3, 4, 5].map((s) => (
                            <Star
                                key={s}
                                className={`w-2.5 h-2.5 sm:w-3 sm:h-3 ${s <= Math.round(product.average_rating) ? "text-amber-400 fill-amber-400" : "text-gray-200"}`}
                            />
                        ))}
                    </div>
                ...
                <div className="flex items-center justify-between mt-auto gap-1">
                    ...
                    {qty > 0 ? (
                        <div
                            className="flex items-center gap-0.5 sm:gap-1 bg-primary-50 rounded-full px-1 py-0.5 flex-shrink-0"
                            onClick={(e) => e.preventDefault()}
                        >
                            <button
                                onClick={handleDecrease}
                                disabled={busy}
                                className="w-5 h-5 sm:w-6 sm:h-6 bg-primary-700 hover:bg-primary-800 text-white rounded-full flex items-center justify-center transition-colors disabled:opacity-50"
                            >
                                <Minus className="w-2.5 h-2.5 sm:w-3 sm:h-3" />
                            </button>
                            <span className="text-xs sm:text-sm font-bold text-primary-700 w-4 sm:w-5 text-center">
                                {qty}
                            </span>
                            <button
                        ...
                        <button
                            ...
                            className="w-7 h-7 sm:w-8 sm:h-8 bg-primary-700 hover:bg-primary-800 text-white rounded-full flex items-center justify-center transition-colors shadow-sm disabled:opacity-50 flex-shrink-0"
                        >
